...
def main(sat_fold, coords_file, out_fold, pollution, delta_pol, gains,
    ...
    satfolders = list(x for x in sat_fold.iterdir() if x.is_dir() and
                      x.name.endswith(".SEN3"))
    ...
    if dem_prods:
        [columns.append(x) for x in ["altitude", "slope", "aspect",
                                     "elevation_variance"]]
    ...
    for location in coords:
        ...
        if incsv.is_file():
            ...
            temp_df = temp_df[columns + alb_columns + rtoa_columns +
                              rbrr_columns + planar_albedo_columns]
